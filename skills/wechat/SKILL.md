...
It becomes the first line; WeChat has no subject. A title with a line break exits `1`.
...
Routing: one bot serves every session in `$ASK_HUMAN_DIR`, and the inbox that is
polling sorts each message into a per-title queue under `wechat/sessions/`. A
...
Fetched batches are first saved under `$ASK_HUMAN_DIR/wechat/recovery/`; the
poller resumes their distribution before fetching again. Each block is appended
to `wechat/history.log` and its destination queue before the cursor advances.
...
| `1`   | Usage, storage or API failure, including queue contention beyond the local budget; stderr starts `wechat: `. Report the error.                                                                                      |
...
One `$ASK_HUMAN_DIR`, one QR login and one bot serve all parallel agent sessions,
told apart by `--title` alone: every session needs its own stable title, reused
for `notify` and `inbox`. Whether one login can serve several directories is
unverified: `getupdates` may consume the cursor server-side. Do not copy
`wechat.json` to another directory; it gets its own setup.
